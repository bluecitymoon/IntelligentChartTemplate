...
<%_ } _%>
<%_ if (service != 'no') { _%>
import <%=packageName%>.service.<%= entityClass %>Service;import <%=packageName%>.repository.<%= entityClass %>Repository;<% } else { %>
...
public class <%= entityClass %>Resource {
    ...
    @GetMapping("/<%= entityApiUrl %>/person/{id}")
    @Timed
    public ResponseEntity<List<<%= instanceType %>>> getAllPerson<%= entityClass %>ByPersonId(@PathVariable Long id, @ApiParam Pageable pageable)
    throws URISyntaxException {

    Page<<%= instanceType %>> page = <%= entityInstance %>Repository.findByPerson_Id(id, pageable);
